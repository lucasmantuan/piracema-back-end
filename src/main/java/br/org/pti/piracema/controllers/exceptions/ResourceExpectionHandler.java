package br.org.pti.piracema.controllers.exceptions;
...
public class ResourceExpectionHandler {
    ...
    @ExceptionHandler(DataIntegrationViolation.class)
    public ResponseEntity<StandardError> dataIntegrationViolation(DataIntegrationViolation e,
            HttpServletRequest request) {
        String error = "Wrong format or missing attribute";
        HttpStatus status = HttpStatus.BAD_REQUEST;
        StandardError err = new StandardError(Instant.now(), status.value(), error, e.getMessage(),
                request.getRequestURI());
        return ResponseEntity.status(status).body(err);
